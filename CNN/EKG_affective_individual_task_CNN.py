...
from utils import save_plot_with_timestamp, sliding_window, load_dataset_EKG, sliding_window_no_overlap, train_test_split, train_test_split, train_test_split_subject_holdout, sliding_window_get_sub_id, is_file_empty_or_nonexistent, sliding_window_no_subject_overlap


# os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:27702"

def classify_CNN_Affective_Individual_Task_EKG(path, hidden_size, num_epochs, batch_size, learning_rate, subject_holdout, window_size, gpu, use_wavelet, use_emd):
    ...
    with open(csv_file_path, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([
            time.strftime("%Y-%m-%d %H:%M:%S"), "EKG", subject_holdout_str, hidden_size, num_epochs, 
...
if __name__ == "__main__":
    ...
    sys.exit(classify_CNN_Affective_Individual_Task_EKG(path, hidden_size, num_epochs, batch_size, learning_rate, subject_holdout, window_size, gpu, use_wavelet, use_emd))
