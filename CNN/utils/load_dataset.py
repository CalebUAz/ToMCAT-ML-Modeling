...
def load_dataset_EKG(path):
    print("-------------------------------------------------------------------------------------------------")
    print("Reading EKG affective task data from: {}".format(path))
    print("-------------------------------------------------------------------------------------------------")
    ...
    dfs = [] # List of dataframes
    subject_ids = [] # Subject id for train test split logic
    headers = ["EKG", "image_path", "arousal_score", "valence_score"]

    # drop_headers = ['AFF5h', 'FC1', 'CP5', 'CP1', 'PO9', 'Oz', 'PO10', 'CP6', 'CP2', 'FC2', 'AFF6h']
    
    columns_to_keep = ["EKG", "image_path", "arousal_score", "valence_score"]
    count = 0
    for root, dirs, files in os.walk(directory):
        for folder in dirs:
            if folder.startswith('exp_'):
...
def load_dataset_GSR(path):
    print("-------------------------------------------------------------------------------------------------")
    print("Reading GSR affective task data from: {}".format(path))
    print("-------------------------------------------------------------------------------------------------")
    ...
    dfs = [] # List of dataframes
    subject_ids = [] # Subject id for train test split logic
    headers = ["GSR", "image_path", "arousal_score", "valence_score"]

    # drop_headers = ['AFF5h', 'FC1', 'CP5', 'CP1', 'PO9', 'Oz', 'PO10', 'CP6', 'CP2', 'FC2', 'AFF6h']
    
    columns_to_keep = ["GSR", "image_path", "arousal_score", "valence_score"]
    count = 0
    for root, dirs, files in os.walk(directory):
        for folder in dirs:
            if folder.startswith('exp_'):
